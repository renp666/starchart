...
65001），含非 ASCII 内容的 .bat 在错误代码页下行边界会错位——实测连英文单词
残片都会被当成命令执行。所以三个 .bat 文件保持纯 ASCII，全部双语文案放在
`msg.ps1`（UTF-8 带 BOM，PowerShell 按 BOM 正确解码）中，由 PowerShell 输出
——控制台渲染不受代码页影响，任何环境都不乱码。
