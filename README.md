...
<details>
<summary><b>启动脚本的提示是中英双语，原理是什么</b></summary>

cmd 按「当前代码页」解码批处理文件（双击是 936，配置过 UTF-8 的 PowerShell 是
65001），含中文的 .bat 在错误代码页下行边界会错位，提示文字甚至会被当成命令执行。
所以脚本首行先 `chcp 65001` 切到 UTF-8，再输出中英双语提示（文件本体保存为
UTF-8 无 BOM）。个别老控制台若字体不支持中文，中文会显示为方框但不影响执行；
Python 输出的反馈不受代码页影响。运行 `start-console.bat` 可见完整提示。
