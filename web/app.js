...
const TOOL_NAMES = {
  claude: "Claude Code", codex: "Codex CLI", gemini: "Gemini CLI", qwen: "Qwen Code",
  trae: "Trae", "trae-cn": "Trae CN", vscode: "VS Code", cursor: "Cursor",
  windsurf: "Windsurf", qoder: "Qoder", copilot: "Copilot CLI", aider: "Aider",
  opencode: "OpenCode", codebuddy: "CodeBuddy", kiro: "Kiro CLI", crush: "Crush",
  iflow: "iFlow CLI", workbuddy: "WorkBuddy", zcode: "ZCode", dsh: "DSH",
  pi: "Pi Agent", lingma: "通义灵码", minimax: "MiniMax Code",
};
...
function isNetError(e) {
  return e instanceof TypeError && /fetch|NetworkError|load/i.test(e.message || "");
}
...
async function api(path, body, raw) {
  const opt = body
    ? { method: "POST", body: raw ? body : JSON.stringify(body),
        headers: raw ? { "Content-Type": "application/zip" } : { "Content-Type": "application/json" } }
    : {};
  let resp;
  try {
    ...
    if (isNetError(e)) {
      ...
      probeAlive().then((alive) => { if (!alive) showNetDown(); });
      const err = new Error("请求失败：本地服务暂时没有响应，请点右下角重试或查看服务是否运行");
      err.isNetwork = true;
      throw err;
    }
    throw e;
  ...
  try {
    ...
    probeAlive().then((alive) => { if (!alive) showNetDown(); });
    const err = new Error("本地服务返回了异常内容（可能已重启或不是星图的服务），请点右下角重新连接");
    err.isNetwork = true;
    throw err;
  }
...
function toastAct(msg, label, fn) {
  ...
  clearTimeout(t._timer);
  t._timer = setTimeout(() => { t.innerHTML = ""; t.classList.add("hidden"); }, 8000);
}
...
function getExpanded() {
  try { return new Set(JSON.parse(localStorage.getItem("starchart-expanded") || "[]")); }
...
function getCollapsedRoots() {
  try { return new Set(JSON.parse(localStorage.getItem("starchart-collapsed-roots") || "[]")); }
  catch { return new Set(); }
}
function saveCollapsedRoots(set) {
  localStorage.setItem("starchart-collapsed-roots", JSON.stringify([...set]));
}
let collapsedRoots = getCollapsedRoots();
...
function renderRootHeader(c, collapsed) {
  ...
  hd.addEventListener("keydown", (e) => {
    if (e.key === "Enter" || e.key === " ") { e.preventDefault(); toggleRoot(c.path); }
  });
  return hd;
}
...
function nodeMatches(node, q) {
  ...
  if (node.tags && node.tags.some((t) => String(t).toLowerCase().includes(q))) return true;
...
function renderNode(node, q, isRoot, depth = 1) {
  ...
  row.className = "node-row" + (isProject ? " project" : "") +
  ...
  if (node.marked === "manual") html += `<span class="badge manual">手动</span>`;
  ...
  if (node.git) {
    if (node.git.dirty) html += `<span class="git-dirty" title="有未提交的更改">●</span>`;
    if (node.git.branch) {
  ...
  row.addEventListener("click", (e) => {
    ...
    if (hasKids) {
      ...
      if (e.target.closest(".twisty") || willExpand) {
        if (willExpand) expanded.add(node.path); else expanded.delete(node.path);
  ...
  row.addEventListener("keydown", (e) => {
    ...
    if (e.key === "ArrowDown") { e.preventDefault(); rows[idx + 1]?.focus(); }
      ...
      e.preventDefault(); expanded.add(node.path); saveExpanded(expanded); render(); row.focus();
    } else if (e.key === "ArrowLeft" && hasKids && !isCollapsed) {
      e.preventDefault(); expanded.delete(node.path); saveExpanded(expanded); render(); row.focus();
    } else if (e.key === "Home") { e.preventDefault(); rows[0]?.focus(); }
    else if (e.key === "End") { e.preventDefault(); rows[rows.length - 1]?.focus(); }
    else if (e.key === "Enter") { e.preventDefault(); row.click(); }
  });
  ...
  const visible = (q
    ...
    : node.children || []).filter((c) => c.marked !== "off");  // 排除的不显示
...
function updateViewButton() {
  ...
  if (viewMode === "frequent") {
    b.textContent = "★ 常用";
    b.title = "当前：常用项目（按使用频率排序）—— 点击回到目录树";
  } else {
    b.textContent = "🌳 树";
    b.title = "当前：目录树 —— 点击切换到常用项目";
  }
}
...
function renderFrequent() {
  ...
  (function walk(n) {
    if (!n) return;
    if (n.type === "project" && n.marked !== "off" && usage[n.path]) items.push(n);
...
function applyHash() {
  try {
    ...
    if (q) {
      searchQuery = q;
      const si = $("#search"); if (si) si.value = q;
      const cb = $("#search-clear"); if (cb) cb.style.display = "block";
    }
    if (p.get("view") === "frequent") viewMode = "frequent";
    const path = p.get("path");
    if (path) { selectedPath = path; selectedNode = null; }
  } catch (e) {}
}
...
function renderTree() {
  ...
  const roots = (q
    ...
    : data.tree.children).filter((c) => c.marked !== "off");  // 排除的不显示
  if (q && roots.length === 0) {
    treeEl.innerHTML =
...
function renderDetail(node) {
  ...
  openMoreMenu = null;
  const d = $("#detail");
  if (!node) { d.classList.add("hidden"); return; }
  ...
  meta.innerHTML = `
    ...
    修改：${fmtTime(node.mtime)}${node.fileCount ? `　·　${node.fileCount} 个文件` : ""}${
      node.introSource ? `　·　${{ ai: "AI 生成", static: "README 提取", manual: "手动编辑" }[node.introSource] || node.introSource}` : ""
    }`;
  d.appendChild(meta);
  ...
  if (node.git) {
    ...
    bits.push(g.dirty ? `<span class="dirty">● 有未提交更改</span>`
  ...
  if (node.type === "project") {
    ...
    editBtn.textContent = (node.tags && node.tags.length) || node.note
      ? "✎ 编辑标签 / 备注" : "🏷 添加标签 / 备注";
  ...
  if (node.type === "project") {
    ...
    box.textContent = node.intro || "暂无介绍 —— 点击编辑，或用顶栏「生成介绍」";
    box.title = "点击编辑介绍";
  ...
  if (launchers.length === 1) {
    ...
    launchCard.addEventListener("click", () => showLaunchMenu(launchCard, node, launchers));
  } else {
  ...
  const edList = useAll ? detectedEditors : detectedEditors.filter((e) => specificHints.includes(e.id));
  const agList = useAll ? detectedAgents : detectedAgents.filter((a) => specificHints.includes(a.id));
  const ways = [];
  edList.forEach((ed) => ways.push({
    icon: "✎", mode: "editor", editor: ed, agent: null,
    name: ed.name, action: `用 ${ed.name} 打开`, src: useAll ? "all" : "hint",
  }));
  agList.forEach((ag) => ways.push({
    icon: "🤖", mode: "agent", editor: null, agent: ag,
    name: ag.name, action: `在终端启动 ${ag.name}`, src: useAll ? "all" : "hint",
  }));
  const detectedIds = new Set([...detectedEditors.map((e) => e.id), ...detectedAgents.map((a) => a.id)]);
  const missing = specificHints.filter((h) => !detectedIds.has(h)).map((h) => TOOL_NAMES[h] || h);
  const openCard = document.createElement("div");
  openCard.className = "action-card" + (ways.length ? "" : " disabled");
  let cardSub, cardLabel;
  // 单项：标签直接写动作（「用 Trae 打开」），副标题写来源；多项：标签写分类
  if (ways.length === 1) { cardLabel = ways[0].action; cardSub = specificHints.length ? "项目配置匹配" : "本机已装"; }
  else if (ways.length > 1) {
    cardLabel = "编辑器 / AI";
    cardSub = specificHints.length ? `${ways.length} 项 · 项目配置匹配` : `${ways.length} 项 · 本机已装`;
  } else { cardLabel = "无可用工具"; cardSub = missing.length ? `缺 ${missing.length} 个（未安装）` : "未检测到编辑器 / Agent"; }
  openCard.innerHTML = `<div class="card-icon">${ways.length === 1 ? ways[0].icon : "⇱"}</div>` +
    `<div class="card-label">${esc(cardLabel)}</div><div class="card-sub">${esc(cardSub)}</div>`;
  openCard.title = missing.length ? "项目期待但未安装：" + missing.join("、")
    : (ways.length === 1 ? ways[0].action : "选择用哪个工具打开 / 启动");
  if (ways.length === 1) {
    const w = ways[0];
    openCard.addEventListener("click", () => openPath(node.path, w.mode, w.editor, w.agent));
  } else if (ways.length > 1) {
    openCard.addEventListener("click", () => showOpenMenu(openCard, node, ways, missing));
  }
  ...
  mi("📂 资源管理器", () => {
    const now = Date.now();
    if (now - (lastExplorer[node.path] || 0) < 1500) { toast("资源管理器刚已打开"); return; }
    lastExplorer[node.path] = now;
    openPath(node.path, "explorer");
  });
  mi("⧉ 复制路径", () => {
    navigator.clipboard.writeText(node.path).then(() => toast("已复制路径")).catch(() => toast("复制失败", true));
  });
  ...
  if (node.type === "project") {
    ...
    input.addEventListener("input", () => {
      ...
      ftimer = setTimeout(async () => {
        ...
        try {
          const r = await api(`/api/files?path=${encodeURIComponent(node.path)}` +
          ...
          for (const rel of r.files) {
            ...
            cp.addEventListener("click", () => {
              navigator.clipboard.writeText(node.path + "\\" + rel)
                .then(() => toast("已复制路径")).catch(() => toast("复制失败", true));
            });
            row.appendChild(nm);
            row.appendChild(cp);
      ...
      if (!docCache[node.path]) {
        ...
        try {
          ...
          docCache[node.path] = r.content
            ...
            : { file: "", html: `<span class="hint-line">未找到说明文档</span>` };
        } catch (e) {
          docCache[node.path] = { file: "", html: `<span class="hint-line">读取失败：${esc(e.message)}</span>` };
        }
      }
      const c = docCache[node.path];
      docBox.innerHTML = (c.file ? `<div class="doc-file">来源：${esc(c.file)}</div>` : "") + c.html;
    });
...
function showLaunchMenu(anchor, node, launchers) {
  showMenu(anchor, launchers.map((l) => ({
    label: "▶ " + l.label,
    fn: () => doLaunch(node, l),
  })));
}
function showOpenMenu(anchor, node, ways, missing) {
  ...
  if (hint.length) {
    ...
    hint.forEach((w) => items.push({
      label: `${w.icon} ${w.action}`, fn: () => openPath(node.path, w.mode, w.editor, w.agent),
    }));
  }
  if (rest.length) {
    items.push({ header: "本机已装" });
    rest.forEach((w) => items.push({
      label: `${w.icon} ${w.action}`, fn: () => openPath(node.path, w.mode, w.editor, w.agent),
    }));
  }
  if (missing && missing.length) {
    items.push({ header: "项目期待·未安装" });
    missing.forEach((name) => items.push({ note: `${name}（未检测到，请先安装）` }));
  }
...
function showRowMenu(e, node) {
  ...
  add(node.starred ? "☆ 取消收藏" : "★ 收藏", () => doStar(node, !node.starred));
...
function startLaunchPoll(node) {
  ...
  launchPollTimer = setInterval(async () => {
    const runDiv = $("#run-state-area");
    if (!runDiv || !document.body.contains(runDiv)) { stopLaunchPoll(); return; }
    await refreshLaunch(node, runDiv);
  }, 4000);
}
...
function editIntro(node) {
  ...
  ta.addEventListener("input", () => { save.disabled = false; save.textContent = "保存"; });
  save.addEventListener("click", async () => {
    save.disabled = true;
    save.textContent = "保存中…";
    try {
      const r = await api("/api/intro/manual", { path: node.path, text: ta.value });
...
function editMeta(node) {
  ...
  m.querySelector("#meta-save").addEventListener("click", async () => {
    const tags = m.querySelector("#meta-tags").value
...
async function confirmExclude(node) {
  if (!confirm(`确定排除「${node.name}」？\n排除后不再作为项目显示；可点下面提示里的「撤销」，或在 设置 → 已排除的项目 中恢复。`)) return;
...
function updateStaleBar() {
  ...
  if (!ts) { bar.classList.add("hidden"); return; }
  const hours = (Date.now() - ts) / 3600000;
  if (hours < INDEX_STALE_HOURS) { bar.classList.add("hidden"); return; }
...
async function doScan() {
  ...
  btn.disabled = true;
  btn.textContent = "扫描中…";
  disabledBtns.forEach((b) => { b.disabled = true; });
  bar.classList.remove("hidden");
  ...
  try {
    ...
    render();
    let msg = `扫描完成：项目 ${r.projects} 个（新增 ${r.new}，移除 ${r.removed}）` +
    ...
    btn.disabled = false;
    btn.textContent = "⟳ 扫描";
    disabledBtns.forEach((b) => { b.disabled = false; });
...
async function doGenerate() {
  ...
  let total = pending.length, done = 0, fail = 0, shown = -1;
  ...
  try {
    ...
    for (;;) {
      ...
      if (!s.running) break;
      if (stopFlag) await api("/api/intro/batch/stop", { taskId: tid }).catch(() => {});
      await new Promise((res) => setTimeout(res, 600));
    }
    toast(stopFlag ? `已停止（完成 ${done}，失败 ${fail}）` :
      fail ? `生成完成，${fail} 个失败` : "全部介绍生成完成");
  } catch (e) {
...
function closeModal() {
  ...
  root.innerHTML = "";
  if (modalFocusReturn) { modalFocusReturn.focus(); modalFocusReturn = null; }
}

/* ================ 设置 ================ */

const PROVIDERS = {
  zhipu: { label: "智谱（glm-4-flash 免费）", baseUrl: "https://open.bigmodel.cn/api/paas/v4", model: "glm-4-flash" },
  siliconflow: { label: "硅基流动（Qwen2.5-7B 免费）", baseUrl: "https://api.siliconflow.cn/v1", model: "Qwen/Qwen2.5-7B-Instruct" },
...
function openSettings() {
  ...
  // 标记脏数据
  m.addEventListener("input", () => { modalDirty = true; });
  m.addEventListener("change", () => { modalDirty = true; });
  ...
  const renderTags = () => {
    ...
    bl.forEach((t, i) => {
      ...
      tag.querySelector(".x").addEventListener("click", () => { bl.splice(i, 1); modalDirty = true; renderTags(); });
  ...
  const addBl = () => {
    ...
    if (v && !bl.includes(v)) { bl.push(v); modalDirty = true; renderTags(); }
    blInput.value = "";
  };
  m.querySelector("#set-bl-add").addEventListener("click", addBl);
  blInput.addEventListener("keydown", (e) => { if (e.key === "Enter") { e.preventDefault(); addBl(); } });
  ...
  m.querySelector("#set-save").addEventListener("click", async () => {
    const cfg = {
      ...
      roots: m.querySelector("#set-roots").value.split("\n").map(s => s.trim()).filter(Boolean),
      blacklist: bl,
      api: {
    ...
    try {
      ...
      modalDirty = false;
      closeModal();
      toast("设置已保存" + (cfg.port !== oldPort ? "（端口修改需重启生效）" : ""));
    } catch (e) { toast("保存失败：" + e.message, true); }
  });
}
...
function openExcluded() {
  ...
  const render = () => {
    ...
    const shown = items.filter((n) => !q || (n.name + " " + n.path).toLowerCase().includes(q));
    listEl.innerHTML = "";
    if (!shown.length) {
    ...
    for (const n of shown) {
      ...
      btn.addEventListener("click", async () => {
        ...
        try {
          ...
          toast(`已恢复「${n.name}」${needRescan ? "（重新扫描后完全生效）" : ""}`);
          items = items.filter((x) => x.path !== n.path);
          render();
          updateExcludedCount();
        } catch (e) { toast(e.message, true); btn.disabled = false; btn.textContent = "恢复"; }
      });
  ...
  m.querySelector("#ex-restore-all").addEventListener("click", async () => {
    ...
    for (const n of items) {
      try { await api("/api/mark", { path: n.path, mark: "auto" }); } catch (e) {}
    }
    items = [];
    render();
    updateExcludedCount();
...
const trend = { since: "daily", lang: "", items: [], intros: {}, guides: {},
                digest: "", langs: [], fetchedAt: 0, filter: "", busy: false };
...
function renderAIBlock(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (bullet && !inList) { html += "<ul class='ai-ul'>"; inList = true; }
    ...
    if (bullet) {
      ...
      html += `<div class='ai-kv'><span class='ai-k'>${esc(k)}</span>${esc(rest.join("："))}</div>`;
    } else if (ln.startsWith("推荐：")) {
      html += `<div class='ai-rec'>⭐ ${esc(ln.slice(3))}</div>`;
    } else if (ln.startsWith("本期风向：")) {
      html += `<div class='ai-wind'>${esc(ln.slice(5))}</div>`;
    } else {
      html += `<div class='ai-one'>${esc(ln)}</div>`;
    }
  }
  if (inList) html += "</ul>";
  return html;
...
async function trendLoad(force) {
  ...
  if (listEl) listEl.innerHTML = `<div class="hint">正在拉取 GitHub 趋势…</div>`;
  try {
    const q = `?since=${encodeURIComponent(trend.since)}&lang=${encodeURIComponent(trend.lang)}`
            + (force ? "&refresh=1" : "");
    ...
    if (langSel && !trend.langsFilled && trend.langs.length) {
      ...
      trend.langs.forEach((lb) => {
        ...
        o.value = lb; o.textContent = lb;
        if (lb === trend.lang) o.selected = true;
        langSel.appendChild(o);
      });
    }
    if (metaEl) {
      const when = trend.fetchedAt ? new Date(trend.fetchedAt * 1000).toLocaleString("zh-CN") : "—";
      metaEl.textContent = `共 ${trend.items.length} 条 · 更新于 ${when}`
        + (r.cached ? (r.stale ? "（网络失败，展示旧缓存）" : "（本地缓存）") : "（刚拉取）");
    }
  } catch (e) {
    if (listEl) listEl.innerHTML = `<div class="hint">拉取失败：${esc(e.message)}</div>`;
    toast(e.message, true);
  }
}
...
function trendCardHTML(it) {
  ...
  const sinceLabel = (TREND_SINCE.find((s) => s[0] === trend.since) || ["", ""])[1];
  return `<div class="trend-card" data-name="${esc(it.fullName)}" data-desc="${esc(it.desc || "")}">
    ...
    <div class="tc-acts">
      ...
      <button data-act="zread" title="用 Zread 打开中文项目文档">Zread 解读</button>
...
function trendRender() {
  ...
  const shown = trend.items.filter((it) =>
    !q || (it.fullName + " " + (it.desc || "") + " " + (it.lang || "")).toLowerCase().includes(q));
  if (!shown.length) {
...
function trendRefreshCard(fullName) {
  const card = document.querySelector(`#tr-list .trend-card[data-name="${CSS.escape(fullName)}"]`);
  if (!card) return;
...
async function trendOneIntro(name, desc, btn) {
  ...
  try {
    const r = await api("/api/trending/intro", { fullName: name, desc: desc || "" });
    trend.intros[name] = r.text;
    trendRefreshCard(name);   // 重绘单卡，按钮 label 由状态自然更新为「🔄 重生成」
  } catch (e) {
    toast(e.message, true);
    if (btn) { btn.disabled = false; btn.textContent = "✨ 中文介绍"; }
  }
}
...
async function trendOneGuide(name, desc, btn) {
  if (btn) { btn.disabled = true; btn.textContent = "…"; }
  try {
    const r = await api("/api/trending/guide", { fullName: name, desc: desc || "" });
    trend.guides[name] = r.text;
    showGuide(name, r.text, r.cached);
    trendRefreshCard(name);
  } catch (e) {
    toast(e.message, true);
    if (btn) { btn.disabled = false; btn.textContent = "📖 中文导读"; }
  }
}

/* 阅读栏辅助：只要有一个结果面板可见就给 #tr-rail 挂 .open（窄屏时收起空条带） */
function trendRailOpen() {
  const rail = $("#tr-rail");
  if (!rail) return;
  const anyOpen = !$("#tr-guide-box").classList.contains("hidden")
...
function trendScrollPanel(box) {
  const rail = $("#tr-rail");
  if (!rail) return;
  const target = box.getBoundingClientRect().top - rail.getBoundingClientRect().top + rail.scrollTop - 4;
...
async function trendBatch() {
  if (trendTaskId) { toast("批量生成进行中，可点「停止」后再试"); return; }
  const targets = trend.items.filter((it) => !trend.intros[it.fullName]);
  if (!targets.length) { toast("榜单里的项目都已经有中文介绍了"); return; }
  const bar = $("#tr-progress");
  bar.classList.remove("hidden");
  bar.setAttribute("aria-valuenow", "0");
  $("#tr-progress-text").textContent = `0 / ${targets.length}`;
  try {
    const r = await api("/api/trending/intros", { items: targets.map((it) => ({ fullName: it.fullName, desc: it.desc })) });
    trendTaskId = r.taskId;
    let fails = 0;   // 轮询连续失败计数：网络闪断退避重试，避免进度条永久卡死
    const tick = async () => {
      try {
        const st = await api(`/api/trending/intros?taskId=${encodeURIComponent(trendTaskId)}`);
        fails = 0;
        const pct = Math.round((st.done + st.fail) / st.total * 100);
        $("#tr-progress-text").textContent = `${st.done + st.fail} / ${st.total}（失败 ${st.fail}）`;
        $("#tr-progress-fill").style.width = pct + "%";
        bar.setAttribute("aria-valuenow", String(pct));
        if (st.running) { trendPoll = setTimeout(tick, 800); return; }
        trendPoll = null;
        bar.classList.add("hidden");
        trendTaskId = null;
        ...
        fails += 1;
        if (fails <= 5) { trendPoll = setTimeout(tick, 1500 * fails); return; }
        trendPoll = null; trendTaskId = null;
...
async function trendDigest() {
  ...
  try {
    const r = await api("/api/trending/digest", { since: trend.since, lang: trend.lang });
    trend.digest = r.text;
    const body = box.querySelector(".digest-body");
    if (body) body.innerHTML = renderAIBlock(r.text);
    else box.innerHTML = box.innerHTML.replace(
      '<div class="hint">正在让 AI 通读榜单并总结…（约需十几秒）</div>',
...
async function trendAdHoc() {
  ...
  raw = raw.replace(/^https?:\/\/(www\.)?github\.com\//i, "").replace(/\/+$/, "");
  raw = raw.replace(/\.git$/i, "");
  const segs = raw.split("/").filter(Boolean);
  if (segs.length > 2) raw = segs[0] + "/" + segs[1];
  if (!/^[^/\s]+\/[^/\s]+$/.test(raw)) {
    trend.adhocBusy = false;
    if (go) { go.disabled = false; go.textContent = "📖 中文导读"; }
...
/* ---- 平铺视图：趋势面板占据原项目树区域，不再用弹窗 ---- */
let trendBound = false;   // 视图为静态 HTML，事件只绑一次

function setTrendView(on) {
  $("#trend-view").classList.toggle("hidden", !on);
  $("#tree-panel").classList.toggle("hidden", on);
  if (on) $("#detail").classList.add("hidden");
  else if (selectedNode) $("#detail").classList.remove("hidden");
}
...
function syncTrendUI() {
  ...
  if (filter && filter.value !== (trend.filter || "")) filter.value = trend.filter || "";
}

function setTrendingButtonState(on) {
  const tb = $("#btn-trending");
  if (!tb) return;
  tb.classList.toggle("on", on);
  tb.setAttribute("aria-pressed", String(on));
  tb.title = on ? "收起 GitHub 趋势榜" : "GitHub 趋势榜 · 中文介绍 / 中文导读";
}

function openTrending() {
  setTrendView(true);
  setTrendingButtonState(true);
  syncTrendUI();
  bindTrendEvents();
  trendLoad(false);   // 后端有 30 分钟缓存，通常 <500ms
}
...
function closeTrending() {
  ...
  trendRailOpen();
  setTrendingButtonState(false);
  setTrendView(false);   // 批量任务不中断：轮询继续，回到视图可见进度
}
...
function bindTrendEvents() {
  ...
  $("#tr-back").addEventListener("click", closeTrending);
  ...
  $("#tr-stop").addEventListener("click", async () => {
    ...
    try { await api("/api/trending/intros/stop", { taskId: trendTaskId }); } catch (e) {}
    toast("已请求停止，正在收尾");
  });
  ...
  $("#tr-list").addEventListener("click", (e) => {
    ...
    else if (act === "zread") window.open("https://zread.ai/" + name, "_blank", "noopener");
    else if (act === "copy") {
      navigator.clipboard.writeText("https://github.com/" + name)
        .then(() => toast("已复制仓库地址"))
...
function openBackup() {
  ...
  m.querySelector("#bk-export").addEventListener("click", async () => {
    try {
      const r = await api("/api/backup/export", { dest: m.querySelector("#bk-dest").value.trim() });
...
function applyTheme() {
  ...
  try { localStorage.setItem("starchart-theme", t); } catch (e) {}
}
...
function watchStartupScan() {
  ...
  let seenScan = false;
  let ended = false;
  const stop = () => { clearInterval(tid); clearTimeout(timer); ended = true; };
...
async function init() {
  ...
  // 系统主题切换时，"跟随系统"模式实时生效
  const onSysTheme = () => { if (config.theme === "auto") applyTheme(); };
  if (themeMedia.addEventListener) themeMedia.addEventListener("change", onSysTheme);
  else themeMedia.addListener(onSysTheme);  // 旧版浏览器

  // 检测本机编辑器与 Agent
  api("/api/editors").then((r) => {
  ...
  $("#btn-view").addEventListener("click", () => {
    viewMode = viewMode === "frequent" ? "tree" : "frequent";
    // 切换视图时清空搜索：常用列表本身就是筛选结果，再叠加搜索没有意义
    searchQuery = "";
    $("#search").value = "";
    $("#search-clear").style.display = "none";
    updateViewButton();
  ...
  $("#btn-stopgen").addEventListener("click", () => { stopFlag = true; });
  $("#btn-settings").addEventListener("click", openSettings);
  $("#btn-trending").addEventListener("click", () => {
    if ($("#trend-view").classList.contains("hidden")) openTrending();
    else closeTrending();
  });
  ...
  search.addEventListener("keydown", (e) => {
    if (e.key === "Escape") {
      search.value = ""; searchQuery = ""; clearBtn.style.display = "none";
      render();
    }
    if (e.key === "Enter") {
